...
export default function AuctionList() {
    ...
    // AuctionList에서 필터
    const auctionItems = (items: ProductItem[]) =>
        // items.filter(i => i.itemStatus === 'ON_SALE' && i.auction === true);
        items.filter(i => i.itemStatus === 'ON_SALE');
    ...
    return (
        <div className="product-list container">
            <section>
                ...
                {renderSwiper(auctionItems(latestItems))}
            </section>
            <br/><br/><br/>
            <section>
                <p className="textMain">관심이 많은 상품</p>
                {renderSwiper(auctionItems(favorites))}
            </section>
            <br/><br/><br/>
            <section>
                <p className="textMain">많이 본 상품</p>
                {renderSwiper(auctionItems(views))}
            </section>
            <br/><br/><br/>
            <section>
                <p className="textMain">전체 상품</p>
                <ul className="grid">
                    {auctionItems(allItems).map(item => (
                        <li key={item.id} className="product-card">
                            ...
                            <Link to={`/items/${item.id}`} className="product-link">
                                ...
                                <div className="product-info">
                                    <div className="favorite-and-views">
                                        <span className="count"><MessageCircle size={16}
                                                                               style={{marginRight: 4}}/> {item.chatRoomCount}</span>
                                        <span className="count"><Heart size={16} fill="#999999" color="#999999"
                                                                       style={{marginRight: 4}}/> {item.favoriteCount}</span>
                                        <span className="count"><Eye size={16}
                                                                     style={{marginRight: 4}}/> {item.viewCount}</span>
                                    </div>
                                    <h3 className="product-title">{item.title}</h3>
                                    <p className="product-price">{item.price.toLocaleString()} 원</p>
                                    <div className="product-meta">
