...
// ✅ 수정 제출
  const handleSubmit = async () => {
    ...
    // 기존 이미지는 그대로 다시 append (파일 형태로 변환해서)
    for (const img of existingImages) {
      const response = await fetch(img.url);
      const blob = await response.blob();
      formData.append("images", new File([blob], `existing-${img.id}.jpg`, { type: blob.type }));
    }
        ...
        >
          ...
          <div className={styles.price}>
            ...
            <div className={styles.priceInputWrapper}>
              <input type="text" placeholder="가격을 입력해주세요." value={price} maxLength={12} onChange={handlePriceChange} />
