...
export default function UsedproductList() {
    ...
    // UsedproductList에서 필터
    const saleItems = (items: ProductItem[]) =>
        items.filter(i => i.itemStatus === 'ON_SALE');
        // items.filter(i => i.itemStatus === 'ON_SALE' && i.auction === 0);

    return (
        <div className="product-list container">
            <section>
                <p className="textMain">최신 등록 상품</p>
                {renderSwiper(saleItems(latestItems))}
            </section>
            <br /><br /><br />
            <section>
                <p className="textMain">관심이 많은 상품</p>
                {renderSwiper(saleItems(favorites))}
            </section>
            <br /><br /><br />
            <section>
                <p className="textMain">많이 본 상품</p>
                {renderSwiper(saleItems(views))}
            </section>
            <br /><br /><br />
            <section>
                <p className="textMain">전체 상품</p>
                <ul className="grid">
                    {saleItems(allItems).map(item => (
